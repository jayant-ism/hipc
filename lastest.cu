...
__global__ void make_count (ll* device_edges , ll* device_last_ele ,ll*  device_group ,ll current_cli_size ,ll m ,ll* device_count )
{
...
__global__ void  make_next(ll* device_edges ,ll* device_last_ele ,ll* device_group ,ll current_cli_size ,ll m ,ll* device_count,ll* device_new_group  ,ll* device_new_last )
{
    ...
    if(index < current_cli_size )
    {
        ...
        for(ll i = index ; i<current_cli_size && device_group[index] == device_group[i] ; i++)
        {
            if(check_edge(device_edges  , 0 , m-1 , device_last_ele[index] , device_last_ele[i]))
            {
                device_new_group[starting_ele] = index ; 
                device_new_last[starting_ele] = device_last_ele[i] ; 
                starting_ele++ ; 
            }
...
ll find_kcliq(ll k)
{
    ...
    ll cli = 2 ;
    ll *device_last_ele , *device_group , *device_edges ; 
    
    
    cudaMalloc( &device_edges , sizeof(ll)*m*2) ;
    cudaMalloc( &device_last_ele , sizeof(ll)*current_cli_size) ;
    cudaMalloc( &device_group , sizeof(ll)*current_cli_size) ;
    vector<ll> temp1(current_cli_size)  , temp2(current_cli_size) ; 
    for(ll i=0 ;i <current_cli_size ;i++)
    temp1[i] =host_cli[i*2] ; 
    for(ll i=0 ;i <current_cli_size ;i++)
    temp2[i] =host_cli[i*2+1] ;
    cudaMemcpy(device_edges ,host_edges.data()  , sizeof(ll)*(m*2) , cudaMemcpyHostToDevice )  ;   
    cudaMemcpy(device_last_ele ,temp2.data()  , sizeof(ll)*(current_cli_size) , cudaMemcpyHostToDevice )  ;   
    cudaMemcpy(device_group ,temp1.data()  , sizeof(ll)*(current_cli_size) , cudaMemcpyHostToDevice )  ;   
    ...
    while(cli < k )
    {
        ...
        make_count<<<blocksPerGrid, threadsPerBlock>>> (device_edges , device_last_ele , device_group , current_cli_size , m , device_count );
     ...
     if(cli+1 <k)
     {
        ...
        cudaMalloc( &device_new_group  , sizeof(ll)*new_cli_size) ;
        cudaMalloc( &device_new_last  , sizeof(ll)*new_cli_size) ;

        make_next<<<blocksPerGrid, threadsPerBlock>>> (device_edges , device_last_ele , device_group , current_cli_size , m , device_count, device_new_group  , device_new_last );
